...
def wavve_extractor(url):
    ...
    offset = 0
    while True:
        apiRequest = f"https://apis.wavve.com/fz/vod/programs-contents/{programid}?limit=50&offset={offset}&orderby=old&apikey=E5F3E0D30947AA5440556471321BB6D9&credential=none&device=pc&drm=wm&partner=pooq&pooqzone=none&region=kor&targetage=all"
        ...
        if len(sourceData["cell_toplist"]["celllist"]) == 0:
            break

        if int(sourceData["cell_toplist"]["pagecount"]) > int(sourceData["cell_toplist"]["count"]):
            offset = offset + 50
        else:
            break
