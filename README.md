...
**注意：** 从 Selenium 迁移到 Playwright 后，浏览器用户数据目录从 `Selenium/` 更改为 `Browser/`。如果需要保留登录状态和浏览器数据，请手动将重要数据从旧目录复制到新目录。
...
- `-h, --headless`: 以无头模式运行浏览器（默认为 GUI 模式）
...
- **无头模式（-h/--headless）**: 浏览器在后台运行，不显示窗口，适合自动化脚本和服务器环境
...
python -m tmdb-import -h "http://www.***.com/video.html"  # 无头模式运行
python -m tmdb-import -dh "http://www.***.com/video.html"  # 调试+无头模式
```
...
# 无头模式: python -m tmdb-import -h "https://www.themoviedb.org/tv/203646/season/1?language=zh-CN"
# 组合选项: python -m tmdb-import -dh "https://www.themoviedb.org/tv/203646/season/1?language=zh-CN"
```
...
python -m tmdb-import -h backdrop "https://www.***.com/image.jpg"  # 无头模式处理背景图
```
...
python -m tmdb-import -h poster "https://www.***.com/image.jpg"  # 无头模式处理海报
```
...
python -m tmdb-import -h fitsize 1920*1080 "https://www.***.com/image.jpg"  # 无头模式裁剪
```
